...
class ItemResultReadResponse(BaseModel):
    ...
    run_id: StrictStr = Field(description="Run UUID to which the item belongs")
    external_id: StrictStr = Field(description="The external_id of the item from the user payload")
    custom_metadata: Optional[Dict[str, Any]]
    status: ItemStatus = Field(description=" When the item is not processed yet, the status is set to `pending`.  When the item is successfully finished, status is set to `succeeded`, and the processing results become available for download in `output_artifacts` field.  When the item processing is failed because the provided item is invalid, the status is set to `error_user`. When the item processing failed because of the error in the model or platform, the status is set to `error_system`. When the run is canceled, the status of all pending items is set to either `cancelled_user` or `cancelled_system`.     ")
    #TODO(Andreas: error_code is missing on item model; should be added in openapi.json and regenerated
    error_message: Optional[StrictStr] = None
    #TODO(Andreas): Please have deprecated message attribute removed in api?
    ...
    __properties: ClassVar[List[str]] = ["item_id", "run_id", "external_id", "custom_metadata", "status", "error_message", "message", "terminated_at", "output_artifacts"]
